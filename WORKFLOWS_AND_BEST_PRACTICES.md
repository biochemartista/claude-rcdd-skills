...
Is question simple & factual?
    ...
    │         └─ Root Claude handles directly (NO @orchestrator)
    │         └─ MANDATORY: PubMed or Perplexity search
    │         └─ Output: ≤500 words with IEEE citations
    │
...
```
1. pubmed-database OR perplexity-search (MANDATORY)
   └─ Quick search: "[topic] [key terms]"
   └─ Retrieve up to10 key PMIDs for citation
   └─ Self-verify: Confirm PMIDs exist before citing

2. Root Claude synthesis
   └─ Combine retrieved evidence + knowledge
   └─ Format as flowing prose with [1], [2] IEEE references
   └─ Self-check all citations before delivery
```
...
[References: IEEE format, up to 10 citations]
```
...
```
1. hmdb-database (for metabolite check)
   └─ Query: compound name
   └─ Purpose: Check if compound or metabolites detected in human plasma/blood
   └─ Output: Metabolite status, tissue concentrations (if found)
   └─ Note: Helps evaluate metabolic transformations in humans
...
- **HMDB for metabolite check** - Identifies human plasma/blood metabolites for PK context
